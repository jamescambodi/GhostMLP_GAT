"""
This script implements a training pipeline for the GAT on Pointnet++ features, which utilizes Graph Attention Networks (GATs) for processing multi-view data. It includes dataset loading, training, validation, and model checkpointing functionalities. The Trainer class handles the core training and evaluation process, while command-line arguments provide customization options.
"""

# Key Features:
# - **Seed Setting**: Ensures reproducibility by setting random seeds for multiple libraries.
# - **Training Management**: Handles training loop, learning rate scheduling, and logging.
# - **Validation Accuracy**: Computes overall and per-class accuracy on validation data.
# - **Custom Model Integration**: Uses the `PointViewGAT` model for multi-view learning.
# import numpy as np
...
import math
from PintView_GAT import PointViewGAT
...
if __name__ == '__main__':
    ...
    log_dir = args.name
    cnet_2 = PointViewGAT(args.name, nclasses=40, num_views=args.num_views)
    ...
    #cnet_2.load_state_dict(torch.load('log/model.pth'))
